...
% Apply form defects: waviness and radius variation

% This script is the same as \LatticeLib\scripts\lattice\lattice_build_ideal.m
% apart from the modification on the medial axes, for applying form
% defects. Unique parts are indicated with **** NEW **** and are located
% only in the following two sections.
...
clear
filesave = false;
%% Load defect data **** NEW ****
% Load PDFs of offset and radius variation for inclined and vertical struts.
% This data is calculated in \LatticeLib\scripts\xct\cross_section_analysis.m

inclined_data = load("inclineddata.mat", "offset_pdf", "radii_pdf");
vert_data = load("verticaldata.mat", "offset_pdf", "radii_pdf");

%% Define lattice geometry **** NEW ****

% General parameters
cs = 7;             % cell size
radius = 0.5;       % strut radius
radius_node = 0.75;	% node radius
n = [2 3 4];        % tessellation (y,x,z).
...
% Inclined struts **** NEW ****
line_segments = 25;
[verts_incl, segments_incl] = strutdefine(incl_start, incl_end,...
    line_segments, 'function', inclined_data.offset_pdf);
radii_incl = [radius inclined_data.radii_pdf.random(1, size(verts_incl, 1)-2) radius];

% Vertical struts **** NEW ****
[verts_vertical, segments_vertical] = strutdefine(vert_start, vert_end,...
    line_segments, 'function', vert_data.offset_pdf);
radii_vert = [radius vert_data.radii_pdf.random(1, size(verts_vertical, 1)-2) radius];
